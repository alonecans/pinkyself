...
nuy.on('message-new', async(faa) => {
    try {
		...
		const groupDesc = isGroup ? groupMetadata.desc : nst''
...
*${fake}*`
				...
				break
            case 'pinky':
		   if (!isOwner) return caa.ReplyTroli(from, 'Khusus Owner!' fake)
           await nuy.toggleDisappearingMessages(from)
           caa.ReplyTroli("yahaha")
           break
            case 'owner':
                nuy.sendMessage(from, {displayname: "Nuy", vcard: vcard}, MessageType.contact, { quoted: faa }
            case 'test':
...
*Runtime :* ${caa.runtime(anu)}`
            ...
            case 'bc': 
					if (!isOwner) return caa.ReplyTroli(from, 'Khusus Owner!' fake)
					if (args.length < 1) return caa.ReplyTroli(from, 'Textnya Mana?' fake)
					anu = await nuy.chats.all()
					if (isMedia && !faa.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(faa).replace('quotedM','m')).message.extendedTextMessage.contextInfo : faa
						buff = await nuy.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							nuy.sendMessage(_.jid, buff, image, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: 'BC BY : OWNER', orderTitle: 'BC BY : OWNER', thumbnail: fs.readFileSync(./media/pinky.jpeg), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./media/pinky.jpeg`, 'base64'), caption: `*「 PESAN BROADCAST 」*\n\n${body.slice(4)}`})
						}
						nuy.sendMessage(from, '*Suksess broadcast*',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./media/pinky.jpeg`, 'base64')})
					} else {
						for (let _ of anu) {
						text = `*「 BC BY OWNER 」*\n\n${body.slice(4)}`
							nuy.sendMessage(_.jid, `${text}`, MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./media/pinky.jpeg`, 'base64')})
						}
						nuy.sendMessage(from, '*Suksess broadcast*',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./media/pinky.jpeg`, 'base64')})
					}
					break
