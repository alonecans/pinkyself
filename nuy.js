...
*Runtime :* ${caa.runtime(anu)}`
			...
			case 'imgtag':
				if ((isMedia && !faa.message.videoMessage || isQuotedImage)) {
					...
					caa.reply(from, `Kirim gambar atau reply dengan caption ${prefix}imgtag caption`, fake)
				}
				break
			...
			case 'tahta':
				...
				break
			case 'pubg':
				if (!arg) return caa.reply(from, `Penggunaan ${prefix}pubg teks1|teks2`, fake)
				argz = arg.split("|")
				if (!argz) return caa.reply(from, `Penggunaan ${prefix}pubg teks1|teks2`, fake)
			...
			case 'togif':
				...
				if (faa.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
					...
					caa.reply(from, `Harus sticker bergerak`, fake)
				}
				break
			...
			case 'creategrup': case 'creategroup': case 'createg':
				...
				if (faa.message.extendedTextMessage != undefined){
					...
					caa.reply(from, `Penggunaan ${prefix}creategrup namagrup|@tag`, fake)
				}
				break
			case 'imgtourl':
			...
			case 'setpp': case 'setprofilepicture':
				...
				if (isMedia && !faa.message.videoMessage || isQuotedImage) {
					...
					caa.reply(from, `Kirim gambar atau reply gambar dengan caption ${prefix}setpp`, fake)
				}
				break
			case 'eval':
				if (!nuyfaa) return
